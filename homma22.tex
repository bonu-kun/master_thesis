...
\begin{quote}
 \begin{itemize}
  \item 横軸が，テストデータにおける縮小データの縮小倍率であり，縦軸がテストデータに対する予測のaccuracyとなっている．
  \begin{quote}
   \begin{itemize}
...
図\ref{fig:nearest_300_1std}最近傍補間によって拡大したテストデータに対する予測は，縮小倍率が1/2から1/4の縮小データに対しては使用可能であることが分かる．なお，縮小倍率が1/5の縮小データに対する予測においては，accuracyが0.9を記録するモデルは上位約17\%程しかなく，モデルの学習にかなりの上振れが起こらないと達成できないスコアであることから，使用可能ではないとの判断を行った．

同様に，図\ref{fig:linear_300_1std}よりバイリニア補間によって拡大したテストデータに対する予測は，縮小倍率が1/2から1/4の縮小データに対しては使用可能であり，また図\ref{fig:cubic_300_1std}よりバイキュービック補間によって拡大したテストデータに対する予測は，縮小倍率が1/2から1/4の縮小データに対しては使用可能であることが分かる．\\このことから，内挿方法によらず，学習データとテストデータの解像度差が1/4までであったら，テストデータとして使用可能であることが分かった．
...
標準偏差（図\ref{fig:nearest_300_1std}から図\ref{fig:cubic_300_1std}）と比べると，各縮小倍率間で有意差が生じることから，標準誤差では誤差がかなり小さくなることが分かる．このことから，深層学習における学習は，1実行ごとにかなりのブレが
...
より将来展望に近づくには，「使用枚数が少ない方がよい」というものである．これを検証するため，分類精度の使用画像枚数依存性について調べる．
